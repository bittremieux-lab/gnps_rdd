...
def _load_sample_types(gfop_metadata: pd.DataFrame, simple_complex: str = "all") -> pd.DataFrame:
    """
...
def _validate_groups(gnps_network: pd.DataFrame, groups_included: List[str]) -> None:
    ...
    if invalid_included_groups:
        raise ValueError(f"The following groups in groups_included are invalid: {invalid_included_groups}")
